use crate::{
    components, entity::Entity, events::Events, generational_index::GenerationalIndex, World,
};
use itertools::izip;

pub(crate) fn cleanup_grabs<const ENTITY_COUNT: usize>(world: &mut World<ENTITY_COUNT>) -> Events {
    for (index, (entity, generation)) in izip!(
        ...
        if entity.has_grab() && entity.has_generation() {
            unsafe {
                world
                    ...
                    .deallocate_unchecked(GenerationalIndex {
                        index,
